...
class AgoraSurfaceView(
  ...
  private fun setData(data: Map<*, *>) {
    ...
    getEngine()?.let { view.setData(it, getVideoManager(), getFURenderer(), channel, (data["uid"] as Number).toInt()) }
  }
  ...
  private fun getVideoManager(): CameraVideoManager? {
    return rtcEnginePlugin.videoManager()
  }

  private fun getFURenderer(): FURenderer? {
    return rtcEnginePlugin.fuRenderer()
  }
