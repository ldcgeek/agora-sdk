    ...
    RtcEngineStreamMessageInterface
{
    ...
    func takePicture(_ params: NSDictionary, _ callback: Callback)
}
...
@objc
class RtcEngineManager: NSObject, RtcEngineInterface, OnPictureListener {
    ...
    private(set) var capturerManager: CapturerManager?
    private var videoProcessingManager: VideoProcessingManager?
    private var takePictureFilter: TakePictureFilter?
    private var takePictureCallback: Callback?
    ...
    @objc func create(_ params: NSDictionary, _ callback: Callback) {
        delegate = RtcEngineEventHandler { [weak self] in
            if ($0 == "joinChannelSuccess" || $0 == "userJoined" || $0 == "userOffline" || $0 == "leaveChannel") {
                print("\($0): \($1!)")
            }
            self?.emitter($0, $1)
        }
        engine = AgoraRtcEngineKit.sharedEngine(with: mapToRtcEngineConfig(params["config"] as! Dictionary), delegate: delegate)
        let encoderConfig = AgoraVideoEncoderConfiguration(size: AgoraVideoDimension1280x720, frameRate: AgoraVideoFrameRate.fps30, bitrate: AgoraVideoBitrateStandard, orientationMode: AgoraVideoOutputOrientationMode.fixedPortrait)
        engine!.setVideoEncoderConfiguration(encoderConfig)
        engine!.setParameters("{\"che.video.zerocopy\":true}")
        initCapturerManager(fua:params["fua"] as! String, bsJson:params["bsJson"] as! String)
        callback.code(engine?.setAppType(AgoraRtcAppType(rawValue: (params["appType"] as! NSNumber).uintValue)!))
    }
    
    @objc func initCapturerManager(fua: String, bsJson: String) {
        let beautySetting = BLMBeautySetting.initWithJson(bsJson) as! BLMBeautySetting
        let fua1 = String(fua.suffix(from: fua.index(fua.startIndex, offsetBy: 10)))
        let fuManager = FUManager.share()
        fuManager?.initFURenderer(fua1)
        beautySetting.toFUManager(fuManager)
        fuManager?.enabled = true
        fuManager?.flipx = true
        fuManager?.isRender = true
        fuManager?.loadFilter()
        fuManager?.setBeautyParameters()
        
        takePictureFilter = TakePictureFilter()
        takePictureFilter?.picListener = self
        videoProcessingManager = VideoProcessingManager()
        videoProcessingManager?.addVideoFilter(takePictureFilter!)
        videoProcessingManager?.addVideoFilter(fuManager!)
        
        let videoConfig = AGMCapturerVideoConfig.default()
//        videoConfig.sessionPreset = AVCaptureSession.Preset.hd1280x720.rawValue as NSString
        videoConfig.fps = 30
        videoConfig.pixelFormat = AGMVideoPixelFormat.NV12
        capturerManager = CapturerManager.init(videoConfig: videoConfig, delegate: videoProcessingManager!)
    }
    ...
    @objc func takePicture(_ params: NSDictionary, _ callback: Callback) {
        let filepath = params["filepath"] as! String
        takePictureCallback = callback
        takePictureFilter?.takePicture(filepath)
    }
    
    @objc func onPicutre(_ pic: String) {
        print("onPicture \(pic)")
        takePictureCallback?.success(pic)
    }
