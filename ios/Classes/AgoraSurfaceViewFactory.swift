...
class AgoraSurfaceView: NSObject, FlutterPlatformView {
    ...
    func setData(_ data: NSDictionary) {
        ...
        if let `engine` = engine {
            _view.setData(engine, capturerManager, channel, (data["uid"] as! NSNumber).uintValue)
        }
    }
    ...
    private var capturerManager: CapturerManager? {
        return rtcEnginePlugin?.capturerManager
    }
